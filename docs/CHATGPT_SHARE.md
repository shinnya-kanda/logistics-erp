...
- **Actual** の最小実装は Phase 2 SQL + `processScanInput`（`scan_events` / progress / issues）。**Phase 2.2** で `driver-app` に手入力 scanner shell（`POST /scans`）あり。カメラ・本格オフラインは未実装。
...
- **Phase 2.4**: [phase2-4-scan-contract-tests.md](./phase2-4-scan-contract-tests.md)（Vitest・`SCAN_CONTRACT_TEST_DATABASE_URL`）
...
| 順序目安 | ファイル | 内容 |
|----------|----------|------|
| 前提 | （外部）`shipments` テーブルが存在すること | `create_inventory_and_stock_movements.sql` が shipments を参照 |
...
DB 変更は packages/db/sql のマイグレーションで行うこと。
```
...
- **未実装寄り**: `scan_events`, `shipment_item_progress`, `shipment_item_issues`, 検証エンジン、WMS ロケーション本格マスタ
- **API が二系統**: `apps/api` と `services/api` が両方ある。どちらを触るか README・用途で確認すること。
...
共有時点のコミットメッセージに Phase1 / Supabase 接続まわりが含まれることが多い。
