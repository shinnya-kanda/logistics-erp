...
function extractCompareGovernanceSemanticResilience(
  value: unknown,
): InventoryCompareGovernanceSemanticResilienceMetadata | null {
  if (!value || typeof value !== "object") return null;
  ...
  if (
    !candidate.compareGovernanceSemanticResilience ||
    typeof candidate.compareGovernanceSemanticResilience !== "object"
  ) {
    return null;
  }

  const semanticResilience =
    candidate.compareGovernanceSemanticResilience as Partial<InventoryCompareGovernanceSemanticResilienceMetadata>;
  return typeof semanticResilience.governanceSemanticResilience === "string" &&
    typeof semanticResilience.semanticResilienceText === "string"
    ? (semanticResilience as InventoryCompareGovernanceSemanticResilienceMetadata)
    : null;
}

function extractCompareGovernanceSemanticIntegrityBoundary(
  value: unknown,
): InventoryCompareGovernanceSemanticIntegrityBoundaryMetadata | null {
  if (!value || typeof value !== "object") return null;

  const candidate = value as {
    readonly compareGovernanceSemanticIntegrityBoundary?: unknown;
  };
  if (
    !candidate.compareGovernanceSemanticIntegrityBoundary ||
    typeof candidate.compareGovernanceSemanticIntegrityBoundary !== "object"
  ) {
    return null;
  }

  const semanticIntegrityBoundary =
    candidate.compareGovernanceSemanticIntegrityBoundary as Partial<InventoryCompareGovernanceSemanticIntegrityBoundaryMetadata>;
  return typeof semanticIntegrityBoundary.governanceSemanticIntegrityBoundary ===
    "string" &&
    typeof semanticIntegrityBoundary.semanticIntegrityBoundaryText === "string"
    ? (semanticIntegrityBoundary as InventoryCompareGovernanceSemanticIntegrityBoundaryMetadata)
    : null;
}

function extractCompareGovernanceSemanticRecoverability(
  value: unknown,
): InventoryCompareGovernanceSemanticRecoverabilityMetadata | null {
  if (!value || typeof value !== "object") return null;
  ...
  if (
    !candidate.compareGovernanceSemanticRecoverability ||
    typeof candidate.compareGovernanceSemanticRecoverability !== "object"
  ) {
    return null;
  }

  const semanticRecoverability =
    candidate.compareGovernanceSemanticRecoverability as Partial<InventoryCompareGovernanceSemanticRecoverabilityMetadata>;
  return typeof semanticRecoverability.governanceSemanticRecoverability === "string" &&
    typeof semanticRecoverability.semanticRecoverabilityText === "string"
    ? (semanticRecoverability as InventoryCompareGovernanceSemanticRecoverabilityMetadata)
    : null;
}
...
function extractCompareGovernanceSemanticDegradationTolerance(
  value: unknown,
): InventoryCompareGovernanceSemanticDegradationToleranceMetadata | null {
  if (!value || typeof value !== "object") return null;

  const candidate = value as {
    readonly compareGovernanceSemanticDegradationTolerance?: unknown;
  };
  if (
    !candidate.compareGovernanceSemanticDegradationTolerance ||
    typeof candidate.compareGovernanceSemanticDegradationTolerance !== "object"
  ) {
    return null;
  }

  const semanticDegradationTolerance =
    candidate.compareGovernanceSemanticDegradationTolerance as Partial<InventoryCompareGovernanceSemanticDegradationToleranceMetadata>;
  return typeof semanticDegradationTolerance.governanceSemanticDegradationTolerance ===
    "string" &&
    typeof semanticDegradationTolerance.semanticDegradationToleranceText === "string"
    ? (semanticDegradationTolerance as InventoryCompareGovernanceSemanticDegradationToleranceMetadata)
    : null;
}

function extractCompareGovernanceSemanticSurvivability(
  value: unknown,
): InventoryCompareGovernanceSemanticSurvivabilityMetadata | null {
  if (!value || typeof value !== "object") return null;
  ...
  if (
    !candidate.compareGovernanceSemanticSurvivability ||
    typeof candidate.compareGovernanceSemanticSurvivability !== "object"
  ) {
    return null;
  }

  const semanticSurvivability =
    candidate.compareGovernanceSemanticSurvivability as Partial<InventoryCompareGovernanceSemanticSurvivabilityMetadata>;
  return typeof semanticSurvivability.governanceSemanticSurvivability === "string" &&
    typeof semanticSurvivability.semanticSurvivabilityText === "string"
    ? (semanticSurvivability as InventoryCompareGovernanceSemanticSurvivabilityMetadata)
    : null;
}

function extractCompareGovernanceSemanticSustainability(
  value: unknown,
): InventoryCompareGovernanceSemanticSustainabilityMetadata | null {
  if (!value || typeof value !== "object") return null;
  ...
  if (
    !candidate.compareGovernanceSemanticSustainability ||
    typeof candidate.compareGovernanceSemanticSustainability !== "object"
  ) {
    return null;
  }

  const semanticSustainability =
    candidate.compareGovernanceSemanticSustainability as Partial<InventoryCompareGovernanceSemanticSustainabilityMetadata>;
  return typeof semanticSustainability.governanceSemanticSustainability ===
    "string" && typeof semanticSustainability.semanticSustainabilityText === "string"
    ? (semanticSustainability as InventoryCompareGovernanceSemanticSustainabilityMetadata)
    : null;
}

function extractCompareGovernanceSemanticMaintainability(
  value: unknown,
): InventoryCompareGovernanceSemanticMaintainabilityMetadata | null {
  if (!value || typeof value !== "object") return null;
  ...
  if (
    !candidate.compareGovernanceSemanticMaintainability ||
    typeof candidate.compareGovernanceSemanticMaintainability !== "object"
  ) {
    return null;
  }

  const semanticMaintainability =
    candidate.compareGovernanceSemanticMaintainability as Partial<InventoryCompareGovernanceSemanticMaintainabilityMetadata>;
  return typeof semanticMaintainability.governanceSemanticMaintainability ===
    "string" && typeof semanticMaintainability.semanticMaintainabilityText === "string"
    ? (semanticMaintainability as InventoryCompareGovernanceSemanticMaintainabilityMetadata)
    : null;
}

function extractCompareGovernanceSemanticEvolvability(
  value: unknown,
): InventoryCompareGovernanceSemanticEvolvabilityMetadata | null {
  if (!value || typeof value !== "object") return null;
  ...
  if (
    !candidate.compareGovernanceSemanticEvolvability ||
    typeof candidate.compareGovernanceSemanticEvolvability !== "object"
  ) {
    return null;
  }

  const semanticEvolvability =
    candidate.compareGovernanceSemanticEvolvability as Partial<InventoryCompareGovernanceSemanticEvolvabilityMetadata>;
  return typeof semanticEvolvability.governanceSemanticEvolvability ===
    "string" && typeof semanticEvolvability.semanticEvolvabilityText === "string"
    ? (semanticEvolvability as InventoryCompareGovernanceSemanticEvolvabilityMetadata)
    : null;
}
...
export function InventoryIntegritySection() {
  ...
  const [
    ...
    setCompareGovernanceSemanticResilience,
  ] = useState<InventoryCompareGovernanceSemanticResilienceMetadata | null>(null);
  const [
    compareGovernanceSemanticIntegrityBoundary,
    setCompareGovernanceSemanticIntegrityBoundary,
  ] = useState<InventoryCompareGovernanceSemanticIntegrityBoundaryMetadata | null>(null);
  const [
  ...
  const [
    ...
    setCompareGovernanceSemanticDegradationTolerance,
  ] = useState<InventoryCompareGovernanceSemanticDegradationToleranceMetadata | null>(null);
  const [
    compareGovernanceSemanticSurvivability,
    setCompareGovernanceSemanticSurvivability,
  ] = useState<InventoryCompareGovernanceSemanticSurvivabilityMetadata | null>(null);
  const [
    compareGovernanceSemanticSustainability,
    setCompareGovernanceSemanticSustainability,
  ] = useState<InventoryCompareGovernanceSemanticSustainabilityMetadata | null>(null);
  const [
    compareGovernanceSemanticMaintainability,
    setCompareGovernanceSemanticMaintainability,
  ] = useState<InventoryCompareGovernanceSemanticMaintainabilityMetadata | null>(null);
  const [
    compareGovernanceSemanticEvolvability,
    setCompareGovernanceSemanticEvolvability,
  ] = useState<InventoryCompareGovernanceSemanticEvolvabilityMetadata | null>(null);
  ...
  useEffect(() => {
    ...
    if (!token) {
      ...
      setCompareGovernanceSemanticDegradationTolerance(null);
      setCompareGovernanceSemanticSurvivability(null);
      setCompareGovernanceSemanticSustainability(null);
      setCompareGovernanceSemanticMaintainability(null);
      setCompareGovernanceSemanticEvolvability(null);
      return;
    }
    ...
    async function loadCompareVisibility() {
      try {
        ...
        if (!response.ok) {
          ...
          setCompareGovernanceSemanticResilience(
            extractCompareGovernanceSemanticResilience(responseBody),
          );
          setCompareGovernanceSemanticIntegrityBoundary(
            extractCompareGovernanceSemanticIntegrityBoundary(responseBody),
          );
          setCompareGovernanceSemanticRecoverability(
          ...
          setCompareGovernanceSemanticDegradationTolerance(
            extractCompareGovernanceSemanticDegradationTolerance(responseBody),
          );
          setCompareGovernanceSemanticSurvivability(
            extractCompareGovernanceSemanticSurvivability(responseBody),
          );
          setCompareGovernanceSemanticSustainability(
            extractCompareGovernanceSemanticSustainability(responseBody),
          );
          setCompareGovernanceSemanticMaintainability(
            extractCompareGovernanceSemanticMaintainability(responseBody),
          );
          setCompareGovernanceSemanticEvolvability(
            extractCompareGovernanceSemanticEvolvability(responseBody),
          );
          return;
        }
    ...
    <section style={styles.panel} aria-labelledby="inventory-integrity-heading">
      ...
      <div style={{ ...styles.notice, ...styles.neutralNotice }}>
        <div style={styles.summaryPanel}>
          ...
          {compareGovernanceSemanticEvolvability ? (
            <>
              <span style={styles.summaryText}>
                {compareGovernanceSemanticEvolvability.semanticEvolvabilityText}
              </span>
              <span style={styles.supportingText}>
                compare governance semantic evolvability:{" "}
                {compareGovernanceSemanticEvolvability.governanceSemanticEvolvability} /{" "}
                {compareGovernanceSemanticEvolvability.semanticEvolvabilityReason}
              </span>
              <span style={styles.supportingText}>
                source: {compareGovernanceSemanticEvolvability.semanticEvolvabilitySource} /
                signals:{" "}
                {readableSignals(
                  compareGovernanceSemanticEvolvability.semanticEvolvabilitySignals,
                )}
              </span>
            </>
          ) : null}
          {compareGovernanceSemanticMaintainability ? (
            <>
              <span style={styles.summaryText}>
                {compareGovernanceSemanticMaintainability.semanticMaintainabilityText}
              </span>
              <span style={styles.supportingText}>
                compare governance semantic maintainability:{" "}
                {
                  compareGovernanceSemanticMaintainability.governanceSemanticMaintainability
                } / {compareGovernanceSemanticMaintainability.semanticMaintainabilityReason}
              </span>
              <span style={styles.supportingText}>
                source: {compareGovernanceSemanticMaintainability.semanticMaintainabilitySource} /
                signals:{" "}
                {readableSignals(
          ...
          {compareGovernanceSemanticSustainability ? (
            <>
              <span style={styles.summaryText}>
                {compareGovernanceSemanticSustainability.semanticSustainabilityText}
              </span>
              <span style={styles.supportingText}>
                compare governance semantic sustainability:{" "}
                {
                  compareGovernanceSemanticSustainability.governanceSemanticSustainability
                } / {compareGovernanceSemanticSustainability.semanticSustainabilityReason}
              </span>
              <span style={styles.supportingText}>
                source: {compareGovernanceSemanticSustainability.semanticSustainabilitySource} /
                signals:{" "}
                {readableSignals(
          ...
          {compareGovernanceSemanticSurvivability ? (
            <>
              <span style={styles.summaryText}>
                {compareGovernanceSemanticSurvivability.semanticSurvivabilityText}
              </span>
              <span style={styles.supportingText}>
                compare governance semantic survivability:{" "}
                {
                  compareGovernanceSemanticSurvivability.governanceSemanticSurvivability
                } / {compareGovernanceSemanticSurvivability.semanticSurvivabilityReason}
              </span>
              <span style={styles.supportingText}>
                source: {compareGovernanceSemanticSurvivability.semanticSurvivabilitySource} /
                signals:{" "}
                {readableSignals(
          ...
          {compareGovernanceSemanticDegradationTolerance ? (
            <>
              <span style={styles.summaryText}>
                {
                  compareGovernanceSemanticDegradationTolerance.semanticDegradationToleranceText
                }
              </span>
              <span style={styles.supportingText}>
                compare governance semantic degradation tolerance:{" "}
                {
                  compareGovernanceSemanticDegradationTolerance.governanceSemanticDegradationTolerance
                } /{" "}
                {
                  compareGovernanceSemanticDegradationTolerance.semanticDegradationToleranceReason
                }
              </span>
              <span style={styles.supportingText}>
                source:{" "}
                {
                  compareGovernanceSemanticDegradationTolerance.semanticDegradationToleranceSource
                }{" "}
                / signals:{" "}
                {readableSignals(
                  compareGovernanceSemanticDegradationTolerance.semanticDegradationToleranceSignals,
                )}
              </span>
            </>
          ) : null}
          ...
          {compareGovernanceSemanticRecoverability ? (
            <>
              <span style={styles.summaryText}>
                {compareGovernanceSemanticRecoverability.semanticRecoverabilityText}
              </span>
              <span style={styles.supportingText}>
                compare governance semantic recoverability:{" "}
                {
                  compareGovernanceSemanticRecoverability.governanceSemanticRecoverability
                } / {compareGovernanceSemanticRecoverability.semanticRecoverabilityReason}
              </span>
              <span style={styles.supportingText}>
                source:{" "}
                {compareGovernanceSemanticRecoverability.semanticRecoverabilitySource} / signals:{" "}
                {readableSignals(
                  compareGovernanceSemanticRecoverability.semanticRecoverabilitySignals,
                )}
              </span>
            </>
          ) : null}
          {compareGovernanceSemanticIntegrityBoundary ? (
            <>
              <span style={styles.summaryText}>
                {compareGovernanceSemanticIntegrityBoundary.semanticIntegrityBoundaryText}
              </span>
              <span style={styles.supportingText}>
                compare governance semantic integrity boundary:{" "}
                {
                  compareGovernanceSemanticIntegrityBoundary.governanceSemanticIntegrityBoundary
                } /{" "}
                {compareGovernanceSemanticIntegrityBoundary.semanticIntegrityBoundaryReason}
              </span>
              <span style={styles.supportingText}>
                source:{" "}
                {compareGovernanceSemanticIntegrityBoundary.semanticIntegrityBoundarySource}{" "}
                / signals:{" "}
                {readableSignals(
                  compareGovernanceSemanticIntegrityBoundary.semanticIntegrityBoundarySignals,
                )}
              </span>
            </>
          ) : null}
          {compareGovernanceSemanticResilience ? (
            <>
              <span style={styles.summaryText}>
                {compareGovernanceSemanticResilience.semanticResilienceText}
              </span>
              <span style={styles.supportingText}>
                compare governance semantic resilience:{" "}
                {
                  compareGovernanceSemanticResilience.governanceSemanticResilience
                } / {compareGovernanceSemanticResilience.semanticResilienceReason}
              </span>
              <span style={styles.supportingText}>
                source:{" "}
                {compareGovernanceSemanticResilience.semanticResilienceSource} / signals:{" "}
                {readableSignals(
                  compareGovernanceSemanticResilience.semanticResilienceSignals,
                )}
              </span>
            </>
          ) : null}
      ...
      <div style={{ ...styles.notice, ...styles.neutralNotice }}>
        ...
        {compareGovernanceSemanticConvergence
          ...
          : ""}
        {compareGovernanceSemanticResilience
          ? ` governance semantic resilience ${compareGovernanceSemanticResilience.governanceSemanticResilience}.`
          : ""}
        {compareGovernanceSemanticIntegrityBoundary
          ? ` governance semantic integrity boundary ${compareGovernanceSemanticIntegrityBoundary.governanceSemanticIntegrityBoundary}.`
          : ""}
        {compareGovernanceSemanticRecoverability
        ...
        {compareGovernanceSemanticObservabilityContinuity
          ...
          : ""}
        {compareGovernanceSemanticDegradationTolerance
          ? ` governance semantic degradation tolerance ${compareGovernanceSemanticDegradationTolerance.governanceSemanticDegradationTolerance}.`
          : ""}
        {compareGovernanceSemanticSurvivability
          ? ` governance semantic survivability ${compareGovernanceSemanticSurvivability.governanceSemanticSurvivability}.`
          : ""}
        {compareGovernanceSemanticSustainability
          ? ` governance semantic sustainability ${compareGovernanceSemanticSustainability.governanceSemanticSustainability}.`
          : ""}
        {compareGovernanceSemanticMaintainability
          ? ` governance semantic maintainability ${compareGovernanceSemanticMaintainability.governanceSemanticMaintainability}.`
          : ""}
        {compareGovernanceSemanticEvolvability
          ? ` governance semantic evolvability ${compareGovernanceSemanticEvolvability.governanceSemanticEvolvability}.`
          : ""}
      </div>
            ...
            >
              ...
              {projection.metadata.compareGovernanceSemanticDegradationTolerance ? (
                <p style={styles.description}>
                  ...
                  {
                    projection.metadata.compareGovernanceSemanticDegradationTolerance
                      .semanticDegradationToleranceText
                  }
                </p>
              ) : null}
              ...
              {projection.metadata.compareGovernanceSemanticEvolvability ? (
                <p style={styles.supportingText}>
                  semantic evolvability reason:{" "}
                  {
                    projection.metadata.compareGovernanceSemanticEvolvability
                      .semanticEvolvabilityReason
                  }{" "}
                  / source:{" "}
                  {
                    projection.metadata.compareGovernanceSemanticEvolvability
              ...
              {projection.metadata.compareGovernanceSemanticMaintainability ? (
                <p style={styles.supportingText}>
                  semantic maintainability reason:{" "}
                  {
                    projection.metadata.compareGovernanceSemanticMaintainability
                      .semanticMaintainabilityReason
                  }{" "}
                  / source:{" "}
                  {
                    projection.metadata.compareGovernanceSemanticMaintainability
              ...
              {projection.metadata.compareGovernanceSemanticSustainability ? (
                <p style={styles.supportingText}>
                  semantic sustainability reason:{" "}
                  {
                    projection.metadata.compareGovernanceSemanticSustainability
                      .semanticSustainabilityReason
                  }{" "}
                  / source:{" "}
                  {
                    projection.metadata.compareGovernanceSemanticSustainability
              ...
              {projection.metadata.compareGovernanceSemanticSurvivability ? (
                <p style={styles.supportingText}>
                  semantic survivability reason:{" "}
                  {
                    projection.metadata.compareGovernanceSemanticSurvivability
                      .semanticSurvivabilityReason
                  }{" "}
                  / source:{" "}
                  {
                    projection.metadata.compareGovernanceSemanticSurvivability
              ...
              {projection.metadata.compareGovernanceSemanticDegradationTolerance ? (
                <p style={styles.supportingText}>
                  semantic degradation tolerance reason:{" "}
                  {
                    projection.metadata.compareGovernanceSemanticDegradationTolerance
                      .semanticDegradationToleranceReason
                  }{" "}
                  / source:{" "}
                  {
                    projection.metadata.compareGovernanceSemanticDegradationTolerance
              ...
              {projection.metadata.compareGovernanceSemanticRecoverability ? (
                <p style={styles.supportingText}>
                  semantic recoverability reason:{" "}
                  {
                    projection.metadata.compareGovernanceSemanticRecoverability
              ...
              {projection.metadata.compareGovernanceSemanticIntegrityBoundary ? (
                <p style={styles.supportingText}>
                  semantic integrity boundary reason:{" "}
                  {
                    projection.metadata.compareGovernanceSemanticIntegrityBoundary
                      .semanticIntegrityBoundaryReason
                  }{" "}
                  / source:{" "}
                  {
                    projection.metadata.compareGovernanceSemanticIntegrityBoundary
