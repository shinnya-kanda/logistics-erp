...
serve(async (req) => {
  ...
  try {
    ...
    const { data: rows, error } = await supabase.rpc("create_distributed_inventory_out", {
      p_part_no: partNo,
      p_quantity: quantity,
      p_warehouse_code: guard.warehouseCode,
      p_from_location_codes: [fromLocationCode],
    ...
    if (error) {
      return jsonResponse(
        { ok: false, error: error.message ?? "failed to create inventory out" },
