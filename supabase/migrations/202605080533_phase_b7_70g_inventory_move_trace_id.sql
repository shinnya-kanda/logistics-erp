-- Phase B7-70G: inventory-move trace_id write support
--
-- 方針:
-- - inventory-move の業務単位 trace_id を OUT(from) + IN(to) の両 inventory_transactions に保存する
-- - 既存14引数相当の呼び出しは p_trace_id text default null で互換維持する
-- - inventory-in / inventory-out / pallet_transactions / warehouse_location_history は変更しない
...
drop function if exists public.create_inventory_move(
  text,
  numeric,
  text,
  text,
...
comment on function public.create_inventory_move(
  ...
  'Phase B7-70G: 棚間移動を trace_id 付き OUT(from) + IN(to) の inventory_transactions 2イベントとして同一トランザクションで作成する。inventory_current は直接更新しない。';
