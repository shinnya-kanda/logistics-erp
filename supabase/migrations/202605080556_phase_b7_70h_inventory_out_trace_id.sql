-- Phase B7-70H: inventory-out trace_id write support
--
-- 方針:
-- - inventory-out の業務単位 trace_id を分散 OUT の全 inventory_transactions に保存する
-- - 既存13引数相当の呼び出しは p_trace_id text default null で互換維持する
-- - inventory-in / inventory-move / pallet_transactions / warehouse_location_history は変更しない
...
drop function if exists public.create_distributed_inventory_out(
  text,
  numeric,
  text,
  text[],
...
comment on function public.create_distributed_inventory_out(
  ...
  'Phase B7-70H: inventory_current を配分判断に使い、trace_id 付き OUT inventory_transactions を自動分割 INSERT する。inventory_current は直接更新しない。';
